...
typedef struct VideoState {
    ...
    int av_sync_type;
    int64_t external_clock; /* external clock base */
    int64_t external_clock_time;

    int64_t audio_clock;
    int64_t audio_diff_cum; /* used for AV difference average computation */
    int64_t audio_diff_avg_coef;
    ...
    int64_t video_current_pts_drift;              ///<video_current_pts - time (av_gettime) at which we updated video_current_pts - used to have running video pts
...
static int64_t get_audio_clock(VideoState *is)
{
    int64_t pts;
    ...
    if (bytes_per_sec)
        pts -= av_rescale(hw_buf_size,1, bytes_per_sec);
    return pts;
}

/* get the current video clock value */
static int64_t get_video_clock(VideoState *is)
{
    if (is->paused) {
        return is->video_current_pts;
    } else {
        return av_rescale(is->video_current_pts_drift + av_gettime(),1, 1000000);
    }
}

/* get the current external clock value */
static int64_t get_external_clock(VideoState *is)
{
    int64_t ti;
    ti = av_gettime();
    return is->external_clock + av_rescale((ti - is->external_clock_time) , 1, 1000000);
}
...
static int64_t compute_target_time(int64_t frame_current_pts, VideoState *is)
{
    int64_t delay, sync_threshold, diff;
                             ...
                             int samples_size1, int64_t pts)
{
    ...
    if (((is->av_sync_type == AV_SYNC_VIDEO_MASTER && is->video_st) ||
        ...
        if (diff < AV_NOSYNC_THRESHOLD) {
            is->audio_diff_cum = diff + av_rescale(is->audio_diff_avg_coef , is->audio_diff_cum, 1);
            if (is->audio_diff_avg_count < AUDIO_DIFF_AVG_NB) {
                /* not enough measures to have a correct estimate */
                is->audio_diff_avg_count++;
            } else {
                /* estimate the A-V difference */
                avg_diff = av_rescale(is->audio_diff_cum , (1.0 - is->audio_diff_avg_coef),1);

                if (abs(avg_diff) >= is->audio_diff_threshold) {
                    wanted_size = samples_size + av_rescale(diff * is->audio_st->codec->sample_rate , n); //changed with av_rescale
                    nb_samples = samples_size / n;

                    min_size = ((nb_samples * (100 - SAMPLE_CORRECTION_PERCENT_MAX)) / 100) * n; //these two operations should not be affected by overflow issues
...
static int audio_decode_frame(VideoState *is, int64_t *pts_ptr)
{
...
static int stream_component_open(VideoState *is, int stream_index)
{
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        is->audio_diff_threshold = av_rescale(2 , SDL_AUDIO_BUFFER_SIZE , avctx->sample_rate);
                ...
                if (cur_stream) {
                    if (seek_by_bytes) {
                        ...
                        if (cur_stream->ic->bit_rate)
                            incr *= av_rescale(cur_stream->ic->bit_rate,1,8);
                        else
                            incr *= 180000;
                        pos += incr;
                        ...
                        pos += incr;
                        stream_seek(cur_stream, pos * AV_TIME_BASE, incr * AV_TIME_BASE, 0);
                    }
                }
                break;
            default:
        ...
        case SDL_MOUSEMOTION:
            ...
            if (cur_stream) {
                if(seek_by_bytes || cur_stream->ic->duration<=0){
                    ...
                    tmm = (tns%3600)/60;
                    tss = (tns%60);
                    frac = av_rescale(x,1,cur_stream->width);
                    ns = frac*tns;
                    hh = ns/3600;
                    mm = (ns%3600)/60;
                    ss = (ns%60);
